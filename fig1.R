...
# Find Markers for major cell types (downsampled for efficiency)
...
################################################################################
# Section 3: Subsetting Data for Downstream Analysis
################################################################################

# Split and save major lineages
...
################################################################################
# Section 4: Figure 1 Visualizations (UMAP & Pie Charts)
################################################################################
...
# Pie Chart: Cell number per Dataset
...
pie(myratio, labels = pielabel, radius = 1, clockwise = F,
    main = "Cell number", border = "white", col = colors)
dev.off()

# Pie Chart: Sample number per Dataset
...
pie(myratio_s, labels = pielabel_s, radius = 1, clockwise = FALSE,
    main = "Sample number", border = "white", col = colors)
dev.off()

# Final Cell Type UMAP
...
DimPlot(scRNA, label = FALSE, group.by = 'celltype', 
  ...
  theme_dr(xlength = 0.2, ylength = 0.2, 
           arrow = grid::arrow(length = unit(0.1, "inches"), type = "closed")) +
  theme(aspect.ratio = 1, panel.grid = element_blank())
dev.off()#   theme(panel.grid.major = element_blank(),
